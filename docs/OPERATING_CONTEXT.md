...
- **No claim about a living person beyond what its source supports** — and this binds **generated prose, not only dossiers.** Dossiers: no claim without a citation to the primary record. Summaries, primers, context and synthesis: no characterization of a legal outcome beyond what the source article literally says — "charged" is not "guilty," a settlement is not a finding of liability, an investigation is not a finding, an accusation is not a fact. Enforced in code by `sift-api/tests/test_generation_constraints.py`, which asserts the constraint block is present in all three generators. (*Starbuck v. Google*, MTD denied 24 Jul 2026; *Bouck v. Meta* — §230 does not cover AI-generated content.)

  > **This bullet read "No *dossier* claim" until 2026-08-18, which was the pre-reversal framing.** [`LAUNCH_DECISION_MEMO.md`](./LAUNCH_DECISION_MEMO.md) §2.2(b) inverted the risk ranking on 2026-07-27 — the dossier tables are hand-seeded and sparse and mostly render database fields, while the **summarizer emits unreviewed sentences about named living people across 59 feeds every 30 minutes** — and its item B2 put the constraint into all three generation prompts the same week. So for three weeks the rule was enforced in code on a surface the rule did not say it covered, and the sharper of the two risks was the one this line omitted.
  >
  > **Verified, and only partly** (`sift-api` [#243](https://github.com/kristenmartino/sift-api/issues/243) / [#264](https://github.com/kristenmartino/sift-api/pull/264), 2026-08-18). Across **38 production summaries carrying a legal matter, zero** turn a charge into guilt — measured with a judge calibrated at **12/12** on exactly that failure, so the result means something. **The opposite failure is unmeasured:** a summary that simply drops the source's "alleged" is invisible to that judge (**0/5**, unanimous, under all three readings of the rubric), and the narrower instrument that reaches 3/5 refuses to report a rate until `sift-api/data/eval/legal_review.csv` is adjudicated by hand. **Do not read the clean escalation number as a clean bill of health** — it bounds one half of this constraint, and the likelier half is still open.
...
| ~~Q8~~ | ~~US-only or global civic content?~~ | **CLOSED 2026-08-05 — global.** Entry point is intergovernmental organizations, not more foreign heads of state: a founding treaty is a fixed document at a stable URL, whereas a head of government is evidenced only by a page that names them today. See [`DECISIONS.md` D47](./DECISIONS.md). Note the compliance surface widens with it — §5's living-persons rule now spans jurisdictions with less forgiving defamation regimes than the US. |
